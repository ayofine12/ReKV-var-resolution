...
# declare -a FRAME_SIZES=(56)
# declare -a LOCAL_BLOCK_COUNTS=(32 64 96)
# declare -a RETRIEVE_SIZES=(16 64)
# declare -a FRAME_SIZES=(56)
# declare -a LOCAL_BLOCK_COUNTS=(32 64 96)
# declare -a RETRIEVE_SIZES=(64)
declare -a FRAME_SIZES=(56)
declare -a LOCAL_BLOCK_COUNTS=(64)
...
for frame_size in "${FRAME_SIZES[@]}"; do
  for local_block_count in "${LOCAL_BLOCK_COUNTS[@]}"; do
    for retrieve_size in "${RETRIEVE_SIZES[@]}"; do
      # if [[ "${frame_size}" == "56" && "${local_block_count}" == "64" && "${retrieve_size}" == "16" ]]; then
      #   echo "==== Skipping frame_size=${frame_size}, local_block_count=${local_block_count}, retrieve_size=${retrieve_size} ===="
      #   continue
      # fi
