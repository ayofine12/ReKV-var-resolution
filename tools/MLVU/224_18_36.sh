...
BASE_SAVE_DIR="${BASE_SAVE_DIR:-/mnt/ssd1/mwnoh/var-resolution-mlvu}"
...
for config in "${CONFIGS[@]}"; do
  ...
  echo "==== Running MLVU frame_size=${frame_size}, local_block_count=${local_block_count}, retrieve_size=${retrieve_size}, retrieve_chunk_size=${RETRIEVE_CHUNK_SIZE} on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} ===="
  if [[ -n "${START_VIDEO_ID}" ]]; then
